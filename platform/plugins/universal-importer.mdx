...
description: 'Import sponsors and contacts from Salesforce, HubSpot, Pipedrive, and other CRM systems'
---
...
<Frame>
  <img src="/images/platform/plugins/importer-source-selection.png" alt="Source System Selection" />
</Frame>
...
<Frame>
  <img src="/images/platform/plugins/importer-file-upload.png" alt="File Upload" />
</Frame>
...
<Frame>
  <img src="/images/platform/plugins/importer-field-mapping.png" alt="Field Mapping" />
</Frame>
...
<Frame>
  <img src="/images/platform/plugins/importer-preview.png" alt="Data Preview" />
</Frame>
...
<Frame>
  <img src="/images/platform/plugins/importer-results.png" alt="Import Results" />
</Frame>
...
| `AnnualRevenue` | `sponsorship_value` | ✅ Yes | Mapped to sponsorship value |
| `Description` | `notes` | ✅ Yes | Internal notes/description |
| `Phone` | - | ❌ No | Not imported (use contacts instead) |
| `BillingStreet` | - | ❌ No | Address fields not imported |
| `BillingCity` | - | ❌ No | Address fields not imported |
| `BillingPostalCode` | - | ❌ No | Address fields not imported |
| `BillingCountry` | - | ❌ No | Address fields not imported |

<Info>
**Why are address fields not imported?**

Valiyou focuses on sponsor relationships and contact management. Physical addresses are typically stored at the contact level in Valiyou, not at the sponsor organization level. If you need address information, include it in the contact import or add it manually to individual contacts.
</Info>

**Not Imported (Salesforce Internal Fields)**:
- `Id`, `IsDeleted`, `MasterRecordId` - Salesforce system IDs
- `ParentId` - Parent account relationships
- `Type`, `AccountNumber`, `Sic`, `SicDesc` - Account classification
- `NumberOfEmployees`, `Ownership`, `TickerSymbol` - Company details
- `Rating`, `Site`, `AccountSource` - Account metadata
- `OwnerId`, `CreatedDate`, `CreatedById`, `LastModifiedDate`, `LastModifiedById`, `SystemModstamp` - Salesforce audit fields
- `LastActivityDate`, `Jigsaw`, `JigsawCompanyId` - Activity tracking
- `ShippingStreet`, `ShippingCity`, `ShippingState`, `ShippingPostalCode`, `ShippingCountry` - Shipping address
- `BillingState`, `BillingStateCode`, `BillingCountryCode`, `BillingLatitude`, `BillingLongitude`, `BillingGeocodeAccuracy` - Address metadata
...
| `AccountId` | `sponsor_id` | ✅ Yes | **Auto-linked** to sponsor by account name |
| `MobilePhone` | - | ❌ No | Use primary Phone instead |
| `Salutation` | - | ❌ No | Name prefix not imported |
| `Department` | - | ❌ No | Department not tracked |
...
- `HomePhone`, `OtherPhone`, `Fax` - Additional phone numbers
...
- `MailingStreet`, `MailingCity`, `MailingState`, `MailingPostalCode`, `MailingCountry` - Mailing address
...
```csv
"Name","Website","Industry","AnnualRevenue","Description"
"Acme Corporation","https://acme.com","Technology","1000000","Leading tech company"
"TechCorp","https://techcorp.com","Software","500000","Software solutions provider"
```

**Contact.csv (Contacts)**:
```csv
"FirstName","LastName","Email","Phone","Title","AccountId"
"John","Doe","john@acme.com","+1234567890","CEO","Acme Corporation"
"Jane","Smith","jane@techcorp.com","+0987654321","CTO","TechCorp"
```

<Note>
The `AccountId` column in the Contact CSV must match the `Name` column in the Account CSV for automatic sponsor linking.
</Note>
