---
title: 'Team Management - User Permissions & Access Control | Valiyou'
description: 'Invite users, manage team permissions, and control access with role-based permissions. Assign user roles, manage seat allocation, send invitations, and implement granular access control for secure team collaboration and organization management.'
---

<Frame>
  <img src="/images/platform/team-management/overview.png" alt="Valiyou team management interface showing user invitations, role-based permissions, access control settings, and seat management dashboard" />
</Frame>
...
<AccordionGroup>
  ...
  <Accordion title="What is role-based access control and how does it work in Valiyou?">
    Role-based access control allows you to assign specific permissions to users based on their responsibilities. Valiyou offers granular permissions including Admin, Dashboard, Reports, Sales, Transfer, Marketing, Brand, Users, Billing, Settings, Plugins, Audit Log, API, and Approver. Each permission controls access to specific features and data. Admin permission automatically grants all sub-permissions, while other roles can be assigned individually for precise access control.
  </Accordion>

  <Accordion title="How many users can I invite to my organization?">
    The number of users depends on your plan: Starter plans include 3 seats, Professional plans include 10 seats, and Enterprise plans offer unlimited seats or custom limits. Both active users and pending invitations count towards your seat limit. You can manage seat allocation by removing inactive users or upgrading your plan via Platform → Billing if you need additional capacity.
  </Accordion>

  <Accordion title="Can I change user permissions after they've joined the organization?">
    Yes, you can modify permissions at any time for active users. Find the user card, click "Permissions" to expand the permissions tree, check or uncheck the desired permissions, and confirm the changes. Permission updates apply immediately with optimistic UI updates. Note that only Admins can grant or revoke Admin permission—users with User Management permission cannot modify Admin status.
  </Accordion>

  <Accordion title="What happens when I remove a user from the organization?">
    When you remove a user, they immediately lose access to the organization, but their historical data (valuations, edits, comments) remains intact for audit trail purposes. The user receives a notification email (optional), and the seat is freed up for new invitations. Removed users can be re-invited later if needed, and they will need to accept a new invitation to regain access.
  </Accordion>

  <Accordion title="How do I help team members who forgot their password?">
    Find the user's card in Platform → Users (they must have Active status, not Pending), click the "Send reset link" button, and confirm sending. The user receives a password reset email with a link (usually valid for 1 hour) to reset their password. Users can also request password resets themselves via the login page—this admin feature is helpful when users can't find or didn't receive the reset email.
  </Accordion>

  <Accordion title="What's the difference between Admin and User Management permissions?">
    Admin permission grants full access to all features and the ability to manage all users including granting/revoking Admin status. User Management permission allows inviting users, assigning most permissions, removing users, and sending password reset links, but cannot grant or revoke Admin permission. This separation ensures only Admins can create new Admins, maintaining security and access control hierarchy.
  </Accordion>

  <Accordion title="Why can't pending invitation permissions be modified?">
    Pending invitations show permissions as assigned during invitation, but cannot be modified until the user accepts and becomes Active. This prevents confusion about what permissions users will receive and ensures the invitation email accurately reflects the access level. To change permissions for pending users, you must cancel the invitation and send a new one with the correct permissions.
  </Accordion>

  <Accordion title="How does seat management work for team collaboration?">
    Seats represent the number of team members (active and pending) allowed in your organization. Both active users and pending invitations count towards your seat limit. To optimize seat allocation, regularly review your team list, remove inactive users, cancel unused pending invitations, and upgrade your plan if you need more capacity. The owner always counts as one seat and cannot be removed.
  </Accordion>
