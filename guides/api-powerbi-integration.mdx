...
'og:title': 'Power BI Integration with API | Valiyou'
---
...
<Note>
  **Time Required**: 60-90 minutes (first-time setup) **Prerequisites**: Microsoft Power BI Desktop
...
<Warning>
  **Security**: Use header-based authentication (recommended) or query parameter. Never commit API
  keys to version control or share publicly.
</Warning>
...
<Tip>
  **Finding Club ID**: Your club ID is in Valiyou URL when logged in:
  `valiyou.io/dashboard?club=YOUR_CLUB_ID` or ask support for it.
</Tip>
...
<Tip>
  **Design Best Practice**: Use your brand colors for consistency. Go to View → Themes → Import
  Theme to add your brand palette.
</Tip>
...
<Warning>
  **Gateway Required**: For on-premises Power BI, you need Power BI Gateway installed. Cloud users
  can refresh directly.
</Warning>
